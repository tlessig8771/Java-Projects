...
public class FlowEdge {
    ...
    public FlowEdge(GraphEdge edge) { //modified to accept my GraphEdge input for edges
        if (edge.getStart() < 0) throw new IllegalArgumentException("vertex index must be a non-negative integer");
        if (edge.getEnd() < 0) throw new IllegalArgumentException("vertex index must be a non-negative integer");
        if (!(edge.getBandwidth() >= 0.0)) throw new IllegalArgumentException("Edge capacity must be non-negative");
        this.v         = edge.getStart();
        this.w         = edge.getEnd();  
        this.capacity  = edge.getBandwidth();
        this.flow      = 0.0;
    }
    ...
    public void resetFlow() //added method to reset flow
    {
    	flow = 0;
    }
